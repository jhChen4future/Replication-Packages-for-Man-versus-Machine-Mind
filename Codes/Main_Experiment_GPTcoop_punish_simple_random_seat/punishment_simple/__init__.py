...
class Constants(BaseConstants):
    ...
    INSTRUCTIONS_TEMPLATE = 'punishment_simple/instructions.html'
    punishment_schedule = {
        0: 0,
        1: 1,
        2: 2,
...
class Player(BasePlayer):
    ...
    Train1 = models.IntegerField(
        label='问题1：假设所有玩家（包括你自己）都选择投入0点数至小组公共账户，请问本轮您的收入是多少？')
    Train2 = models.IntegerField(
        label='问题2：假设所有玩家（包括你自己）都选择投入20点数至小组公共账户，请问本轮您的收入是多少？')
    Train3 = models.IntegerField(
        label='问题3：假设小组其他玩家共投入30点数至小组公共账户，您投入0点数至小组公共账户，请问本轮您的收入是多少？')
    Train4 = models.IntegerField(
        label='问题4：假设您投入10点数至小组公共账户，小组其他三名玩家，共投入20点数至小组公共账户，请问本轮您的收入是多少？')
...
class WaitPage2(WaitPage):
    ...
    @staticmethod
    def is_displayed(player):
        return 1< player.round_number < 11
...
class Results(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        if player.round_number == 1:
            participant.punishment_simple = 0
            participant.punishment_simple = participant.punishment_simple + player.payoff
        else:
            participant.punishment_simple = player.payoff + participant.punishment_simple
        if player.round_number == Constants.num_rounds:
            player.payoff = participant.punishment_simple
        print(participant.punishment_simple)
        print(player.payoff)
...
'''     
            ...
            participant.punishment_simple = player.payoff
            participant.payoff = participant.trust + participant.ultimatum + participant.pd + participant.dictator + participant.punishment_simple
