...
class Constants(BaseConstants):
    ...
    sucker_payoff = 10
    defector_payoff = 40
    INSTRUCTIONS_TEMPLATE = 'Repeated_PD8/instructions.html'
...
class Results_Page_R1(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.round_number == Constants.num_rounds:
            ...
            participant.pd8 = player.payoff
            participant.pd8_round = player.round_to_pay
            #PD抽一局
            game_to_pay = random.randint(1, 8)
            string = 'pd' + str(game_to_pay) #pd2 eg
            participant.pd_block = game_to_pay #局数
            participant.pd = getattr(participant,string) #最后抽取的报酬 participant.pd2 pd2内部已抽取过对应轮的结果 抽取轮存储在pd2_round
            player.payoff = getattr(participant, string)
            participant.payoff = participant.trust + participant.ultimatum + participant.pd #到目前为止的总报酬
            string = 'pd' + str(game_to_pay) + '_round'
            participant.pd_round = getattr(participant,string)#对应局数抽取的轮数
    '''
    @staticmethod
    def before_next_page(player : Player, timeout_happened):
        participant = player.participant
        if player.round_number == 1:
            participant.pd8 = 0
            participant.pd8 = participant.pd8 + player.payoff
            participant.pd = 0
        else:
            participant.pd8 = player.payoff + participant.pd8
        if player.round_number == Constants.num_rounds:
            player.payoff = participant.pd8
            for i in range(1,9):
                string = 'pd'+str(i)
                print(string)
                participant.pd = participant.pd+getattr(participant, string)
                print(participant.pd)
            player.payoff = participant.pd
            print('pd:',participant.pd)
            print('pd8：',participant.pd8)
